...
3.  **Expo Prebuild**:
    ...
    *   *Note*: We use `--no-install` to skip `pod install` (iOS) and speed up the process, as we handle dependencies via `pnpm`.
...
*Note*: You will need the physical `release.keystore` file in `android/app/` for the release build to succeed locally, or configure `gradle.properties` to point to it.
